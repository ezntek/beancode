...
# each formatter formats one block and all sub-blocks.
# the top level generated by the formatter should not contain
# any indentation, only sub-blocks within it that have to have indents.
class Formatter:
    ...
    def visit_literal(self, expr: Literal):
        match expr.val.kind:
            case BCPrimitiveType.STRING:
                self.write(f'"{expr.val}"')
            case BCPrimitiveType.CHAR:
                self.write(f"'{str(expr.val)[0]}'")
